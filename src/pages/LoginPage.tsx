...
const LoginPage = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (err.message?.includes('bad_credentials')) setError('Email sau parolă greșită.');
      else if (err.message?.includes('account_locked')) setError('Cont blocat. Încearcă mai târziu.');
      else setError('Eroare la autentificare.');
    }
    setLoading(false);
  };
    ...
    <div className="min-h-screen flex items-center justify-center bg-background px-4">
      ...
      <div className="relative z-10 w-full max-w-sm">
        ...
        <form onSubmit={handleSubmit} className="bg-card border border-border rounded-lg p-6 space-y-4">
          <h2 className="font-display text-lg tracking-wider text-center mb-2">AUTENTIFICARE</h2>
          ...
          <p className="text-xs text-center text-muted-foreground">
            Nu ai cont? <Link to="/register" className="text-crimson hover:underline">Înregistrează-te</Link>
          </p>
        </form>
      </div>
    </div>
